      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-5">
        <div className="bg-white rounded-2xl border border-gray-100 p-12 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm leading-relaxed">
            Thank you for your interest in becoming a Xiao Long Bow reseller.
            We&apos;ll review your application and reach out via SMS soon.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 px-5 py-10">
        <div className="max-w-2xl mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl font-black mb-3">Become a Reseller</h1>
          <p className="text-gray-500 text-base max-w-lg">
            Sell Xiao Long Bow products in your community and earn with every order.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-5 py-10">
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-8">
          {[
            { title: "Earn per order", desc: "Commission on every box sold through your channel." },
            { title: "Sample products", desc: "Free products to taste and share with your customers." },
            { title: "Promo code", desc: "Your own exclusive discount code for your buyers." },
          ].map((perk) => (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl border border-gray-100 p-8 space-y-5">
          ...
          <div>
            <label className={labelClass}>Why do you want to resell?</label>
            <textarea {...register("reason")} rows={4}
              className={`${inputClass} resize-none`}
              placeholder="Tell us about yourself and why you'd be a great reseller..." />
            {errors.reason && <p className="text-[#E83A87] text-xs mt-1.5 font-medium">{errors.reason.message}</p>}
          </div>

          <div>
            <label className={labelClass}>Previous selling experience <span className="normal-case font-normal text-gray-400">(optional)</span></label>
            <textarea {...register("experience")} rows={3}
              className={`${inputClass} resize-none`}
              placeholder="Any experience selling food or similar products..." />
          </div>

          {error && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-[#E83A87] text-sm font-medium">
              {error}
            </div>
          )}
